...
RECIPE_API_KEY = st.secrets["RECIPE_API_KEY"]



client = OpenAI(base_url="https://openrouter.ai/api/v1", api_key=RECIPE_API_KEY)
...
with st.sidebar:
    st.title('Menu')
    r_button= st.radio('',['Home','Add Recipe', 'Search Recipes', 'Cook Now Calculator', 'Import Recipe', 'Ingredient Substitute'])
        ...
        if meals != None:
            ...
            for i in range(len(meals)):
                ...
                for j in range(1, 21):
                    ...
                    ingredient = meals[i][f"strIngredient{j}"]
                    measure = meals[i][f"strMeasure{j}"]

                    if ingredient:
                        ingredients.append(f"{measure} {ingredient}")

                ing_lst = ", ".join(ingredients)


                with st.container(border=True):

                    st.header(meals[i]["strMeal"])

                    st.write("Category:", meals[i]["strCategory"])
                    st.write("Cuisine:", meals[i]["strArea"])

                    st.write("Ingredients:")
                    st.write(ing_lst)

                    if st.button("Import Recipe", key=f"import_{i}"):

                        new_recipe = pd.DataFrame({
                            "recipe_name": [meals[i]["strMeal"]],
                            "recipe_desc": [meals[i]["strArea"]],
                            "recipe_cat": [meals[i]["strCategory"]],
                            "serv_size": [1],
                            "food_rating": [None],
                            "ing_lst": [ing_lst],
                            "prep_time": [0],
                            "difficulty_level": ["Easy"],
                            "cook_steps": [meals[i]["strInstructions"]],
                            "cook_notes": [""],
                            "time_now": [datetime.datetime.now().date()]
                        })
...
if r_button == "Ingredient Substitute":

    st.title("Ingredient Substitute")

    ingredient = st.text_input(
        "What ingredient do you want to replace?",
        key="ingredient_sub"
    )
    ...
    if st.button("Suggest Substitute"):

        prompt = f"""
        I am making {recipe}.
        I want to replace {ingredient}.

        Suggest 3 suitable substitutes for this specific recipe.
        Briefly explain why each substitute would work.
